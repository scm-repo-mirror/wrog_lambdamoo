...
#   respective cmds are encountered.  (If this is not the best order
#   for organizing the output, you will probably want to have all
#   hooks expanding to nothing while sending output to diversions or
#   defining/appending macros as a side effect.)
#
...
m4_define([ax_lp_NTSC], [m4_translit([[$1]],[NTSC],]m4_format([[[
%c #
]]], 9)[)])
...
#      (undef everything defined by _ax_lp_init and friends).
#
...
#       reverse order.
#
# Note that the ordering of :fn, :fnend, and local declarations
# (:vars, :var, :sets, :hashes) for a given cmd matters.  A given
# variable declaration will only be visible in :fn/:fnend and :var
# value expressions that *follow* it.
#
...
#
# Think of these as better versions of m4_substr(), which is mostly
# useless because it does not quote its results.
#
# Both functions rely on the first <N> chars being quote balanced;
# Bad Things will happen otherwise.  ax_lp_strtail *also* requires
# the remaining chars in the string to be quote-balanced as well
# (surprise).
#
# (You should also expect m4_changequote() to totally screw these up,
# so that is something else to *NOT* do.)
