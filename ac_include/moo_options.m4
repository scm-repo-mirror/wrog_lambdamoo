...
# ------------------------------------------------------------------
#  MOO_WRITE_VERSION_OPTIONS(FILENAME)
#    write out the #define macro invocations needed to convert the
#    option values in options.h into the 'options' group for
#    version_source_full.  These only depend on the *types*
#    of the options (and hence do not need to be regenerated
#    by config.status which only ever rewrites options.h with
#    new values).
#
AC_DEFUN([MOO_WRITE_VERSION_OPTIONS],
[[rm -f "$1"
cat <<'MOO_END' >"$1"
/*-----------------------------------------*
 |  Generated by autoconf from options.ac  |
 |  Do your edits there, not here...       |
 *-----------------------------------------*/
@%:@define _DINT1(OPT) _DINT(@%:@OPT,OPT)
@%:@define _DSTR1(OPT) _DSTR(@%:@OPT,OPT)
]MOO_OPTIONS_FOREACH([_MOO_PUT_VSRC_DECL])[
MOO_END
chmod -w "$1"
]])dnl

#
#  _MOO_PUT_VSRC_DECL(NAME,TYPE)
#    emit declaration of NAME as TYPE
m4_define([_MOO_PUT_VSRC_DECL],
[m4_if(m4_index([$2],[ ]),[-1],
    [_MOO_VSRC_BASE([$1],[$2])],
    [_MOO_VSRC_ENUM([$1],[$2])])])dnl
#
#  _MOO_VSRC_BASE(NAME,TYPE) -- TYPE is a base type
#
m4_define([_MOO_VSRC_BASE],
[[@%:@ifdef $1]
  m4_indir([_MOO_VSRC_DEF($2)],[$1])
[@%:@else
  _DNDEF("$1")
@%:@endif
]])dnl
#
m4_define([_MOO_VSRC_DEF(bool)],[[_DDEF("$1")]])dnl
m4_define([_MOO_VSRC_DEF(int)], [[_DINT1($1)]])dnl
m4_define([_MOO_VSRC_DEF(str)], [[_DSTR1($1)]])dnl
#
#  _MOO_VSRC_ENUM(NAME,TYPE) -- TYPE is an enumeration
#
m4_define([_MOO_VSRC_ENUM],
[[@%:@if !defined($1)
  _DNDEF("$1")]
m4_map_args_w([$2],[_MOO_VSRC_ENUM_SINGLE([$1],],[)],[])dnl
[@%:@else
  _DINT1($1)
@%:@endif
]])dnl
#
m4_define([_MOO_VSRC_ENUM_SINGLE],
[[@%:@elif $1 == $2
  _DSTR("$1","$2")
]])dnl
