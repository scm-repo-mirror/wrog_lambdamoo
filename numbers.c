...
numeric_lt_or_eq(int not, Var a, Var b)
{
	...
	ans.v.num =
	    (aceil <= (double)INT32_MIN)      /* aceil <= INT32_MIN  */
	    || ((aceil < -(double)INT32_MIN)  /* aceil <= INT32_MAX  */
		&& (int32_t)aceil <= b.v.num);
    }
    else { /* b.type == TYPE_FLOAT */
	double bfloor = floor(*b.v.fnum);
	ans.v.num =
	    (-(double)INT32_MIN <= bfloor)      /* INT32_MAX < bfloor  */
	    || ((double)INT32_MIN <= bfloor     /* INT32_MIN <= bfloor */
